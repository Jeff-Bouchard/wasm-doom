[DDOOM.html](https://github.com/user-attachments/files/26072654/DDOOM.html)
<!DOCTYPE html>
<!-- saved from url=(0042)http://cantdoevil.com/production_doom.html -->
<html lang="en" style="--banner-height: 84px;"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    ...
    <script src="./Production Decentralized DOOM_files/qrcode.min.js.download"></script>
    <script src="./Production Decentralized DOOM_files/lz-string.min.js.download"></script>
    <style>
        body {
...
<body>
    ...
    <div class="page-grid">
        <div class="column left-column">
            <div id="status-log">
                <p>🎮 Production Decentralized DOOM Loader Initialized...</p>
                <p>📡 Using real EmerDNS domains for asset resolution</p>
            <p><span style="color: #44ff44">[2:06:03 PM] 🎮 Production Decentralized DOOM Loader Ready</span></p><p><span style="color: #44ff44">[2:06:03 PM] 📡 Click "Verify EmerDNS Domains" to begin</span></p></div>
        </div>
        ...
        <div class="column right-column">
            ...
            <fieldset>
                ...
                <button id="verify-domains" type="button">🔍 Verify EmerDNS Domains</button>
                <button id="load-doom" type="button" disabled="">🚀 Load Decentralized DOOM</button>
            </fieldset>
            ...
            <fieldset>
                ...
                <div class="game-controls-row">
                    ...
                    <input type="password" id="wallet-pass" placeholder="••••••" class="flex1">
                </div>
                <div class="game-controls-row">
    ...
    <script>
        ...
        async function resolveEmerDNS(domain) {
            log(`🔍 Resolving ${domain} to IPFS CID`);
            ...
            if (cid) {
                log(`✓ Resolved ${domain} to CID: ${cid}`);
                return cid;
            }
        ...
        function parseDNSResponse(responseBuffer) {
            ...
            try {
                ...
                log(`⚠ DNS response parsing error: ${err.message}`);
            }
        ...
        async function fetchFromIPFS(hash, isText = false, maxRetries = 3) {
            ...
            for (const proxyUrl of proxies) {
                for (let attempt = 0; attempt <= 1; attempt++) {
                    try {
                        if (attempt > 0) {
                            log(`🔄 Retry attempt ${attempt}/1 for ${hash} via ${proxyUrl.split('?')[0]}`);
                            await new Promise(resolve => setTimeout(resolve, 1000));
                        }

                        log(`🔄 Fetching from ipfs.ness.cx via ${proxyUrl.includes('corsproxy') ? 'corsproxy.io' : proxyUrl.includes('codetabs') ? 'codetabs' : 'allorigins'} (NO CORS)...`);
                        ...
                        log(`✅ Downloaded ${hash}: ${data.length || data.byteLength} bytes from ipfs.ness.cx (NO CORS)`);
                        return data;

                    } catch (err) {
                        if (err.name === 'AbortError') {
                            log(`⚠ Request timeout for ${hash} via proxy`);
                        } else {
        ...
        async function verifyDomains() {
            ...
            for (const domain of domains) {
                ...
                try {
                    ...
                    log(`✗ Failed to resolve ${domainValue}: ${error.message}`, true);
            ...
            if (bundleDomainEl && bundleCidEl) {
                ...
                if (bdom) {
                    try {
                        ...
                        log(`✗ Failed to resolve ${bdom}: ${e.message}`, true);
                    }
                }
                if (bundleCid) {
            ...
            verifyBtn.textContent = '🔍 Verify EmerDNS Domains';

            if (allVerified) {
                loadBtn.disabled = false;
                log('🎉 All EmerDNS domains verified successfully!');
            } else {
                log('❌ Some domains failed verification. Cannot load DOOM.', true);
            }
        }
        ...
        async function loadDecentralizedDoom() {
            ...
            try {
                log('🎮 Starting decentralized DOOM load sequence...');

                // Setup canvas with proper DOOM dimensions - CRITICAL for rendering
                log('🖼️ Setting up game canvas...');
                // Get canvas element
                ...
                log(`✓ Canvas setup: ${gameCanvas.width}x${gameCanvas.height} (scaled to ${gameCanvas.style.width}x${gameCanvas.style.height})`);
                ...
                if (bundleCid) {
                    log(`📦 Bundle CID detected: ${bundleCid} — prioritizing directory-based load`);
                    ...
                    try {
                        ...
                        log(`✓ Bundle assets resolved: WAD=${wadPick.name}, JS=${jsPick.name}, WASM=${wasmPick.name}`);
                    } catch (e) {
                        // If WASM not found in bundle, fall back to individual domain for WASM only
                        log(`⚠ WASM not found in bundle (${e.message}). Falling back to domain for WASM only.`);
                        const wasmResolved = await resolveEmerDNS(wasmDomain);
                        wasmHash = wasmResolved;
                    }

                    log(`✓ Bundle assets ready: WAD=${wadData.byteLength} bytes, JS=${jsCode.length} chars`);
                } else {
                    // Fallback: resolve individual EmerDNS domains
                    log(`📡 Resolving EmerDNS domains: ${wadDomain}, ${jsDomain}, ${wasmDomain}`);
                    [wadHash, jsHash, wasmHash] = await Promise.all([
                        resolveEmerDNS(wadDomain),
                        resolveEmerDNS(jsDomain),
                        resolveEmerDNS(wasmDomain)
                    ]);
                    log(`✓ All domains resolved: WAD=${wadHash}, JS=${jsHash}, WASM=${wasmHash}`);
                    ...
                    log(`✓ Assets ready: WAD=${wadData.byteLength} bytes, JS=${jsCode.length} chars`);
                }

                log('⚙️ Starting WASM engine...');
                ...
                window.Module = {
                    ...
                    instantiateWasm: function(imports, successCallback) {
                        (async () => {
                            try {
                                log(`🔩 Fetching WASM via IPFS execution layer (proxied)`);
                                ...
                                log(`✅ WASM loaded from IPFS (proxied)`);
                                successCallback(results.instance);
                            } catch (e) {
                                log(`❌ WASM load failed via IPFS: ${e.message}`, true);
                                throw e;
                            }
                        })();
                    ...
                    onRuntimeInitialized: function() {
                        log('🎉 DOOM runtime initialized successfully!');
                        gameLoaded = true;
                        ...
                        try {
                            ...
                            log(`⚠ Canvas size guard failed: ${e.message}`, true);
                        }
                                    ...
                                    try {
                                        ...
                                        try {
                                            ...
                                            if (ctx) {
                                                ...
                                                log('🧪 Drew diagnostic pattern (no DOOM frames yet). If visible, engine is not rendering.');
                                            } else {
                                                log('⚠ 2D context unavailable for diagnostics');
                                            }
                                        } catch (e2) {
                                            log(`⚠ Diagnostic draw failed: ${e2.message}`);
                                        }
                                        log('❗ No frames from 2D or WebGL. Potential causes:');
                ...
                log('✅ Decentralized DOOM loaded successfully!');

                // Reset button state
                loadBtn.disabled = false;
                loadBtn.textContent = '🎮 DOOM Loaded';

            } catch (error) {
                log(`❌ Failed to load decentralized DOOM: ${error.message}`, true);
                loadBtn.disabled = false;
                loadBtn.textContent = '🚀 Load Decentralized DOOM';
            }
        }
        ...
        function prepareWager() {
            ...
            if (!coinEl || !amountEl || !urlEl || !statusEl) {
                log('❌ Wager UI elements missing', true);
                return;
            }
            ...
            if (!amount || amount <= 0) {
                log('❌ Enter a positive wager amount', true);
                statusEl.style.backgroundColor = '#e74c3c';
                return;
            }
            if (!endpoint) {
                log('❌ Provide a real Wallet/RPC endpoint for the selected coin', true);
                statusEl.style.backgroundColor = '#e74c3c';
                return;
            }
            if (!walletBytes || walletBytes.byteLength === 0) {
                log('❌ Wallet file (.wlt) is required', true);
                statusEl.style.backgroundColor = '#e74c3c';
                return;
            }

            log(`💰 Wager intent: ${amount} ${coin} via ${endpoint}`);
            // Quick local connectivity probe (no CORS requirements)
            pingEndpoint(endpoint).then((ok) => {
                if (ok) {
                    log('🔗 Endpoint appears reachable (ping ok)');
                    statusEl.style.backgroundColor = '#2ecc71';
                } else {
                    log('⚠️ Could not confirm endpoint reachability (timeout). Proceeding requires local wallet.');
            ...
            log('📝 Action: prepare a SIGNED RAW TRANSACTION (no broadcast).');
            log('❗ Needed: exact RPC for NESS/NCH to (1) check funds sufficiency (return yes/no only) and (2) sign raw tx without broadcasting.');
            log('➡ Provide endpoint paths, JSON schema, and any headers so I can wire this end-to-end.');
        }
        ...
        function buildWalletCliPreview(type, name, passphrase, isTemp) {
            ...
            if (passphrase) {
                const masked = '•'.repeat(passphrase.length);
                parts.push(`-p ${masked}`);
            }
        ...
        async function createWallet() {
            try {
                ...
                if (!nameEl || !passEl || !tempEl || !endpointEl || !statusEl || !previewEl) {
                    log('❌ Wallet UI elements missing', true);
                    return;
                }
                ...
                log(`🧰 Creating ${walletTypeDesc}`);

                if (isTemp) {
                    log('⚠️ Temporary wallets self-destruct when the session ends or conditions are met');
                }

                if (!endpoint) {
                    log('⚠️ No wallet RPC endpoint provided in `endpoint-url`. Command preview shown only; no request sent.');
                    return;
                }
                ...
                if (isTemp) {
                    log(`✅ Temporary wallet created successfully`);
                    log(`🔑 Wallet ID: ${out.id || 'N/A'}`);
                    log(`⏰ Auto-destruct conditions: ${out.destructConditions || 'session end'}`);
                } else {
                    log(`✅ Wallet created successfully${out && out.name ? `: ${out.name}` : ''}`);
                }
            } catch (e) {
                const statusEl = document.getElementById('wallet-create-status');
                if (statusEl) {
                ...
                log(`❌ Wallet creation failed: ${e.message}`, true);
            }
        }
        ...
        async function addPrivateKey() {
            try {
                ...
                if (!keyEl || !nameEl || !endpointEl || !statusEl) {
                    log('❌ Add key UI elements missing', true);
                    return;
                }
                ...
                if (!privateKey) {
                    log('❌ Private key required', true);
                    return;
                }

                if (!endpoint) {
                    log('❌ No RPC endpoint provided', true);
                    return;
                }
                ...
                statusEl.className = 'status-indicator';
                statusEl.style.backgroundColor = '#f5a623'; // amber
                log(`🔑 Adding private key to wallet '${walletName}'`);

                // Show CLI preview
                const previewEl = document.getElementById('wallet-cli-preview');
                if (previewEl) {
                    const maskedKey = privateKey.substr(0, 8) + '•'.repeat(Math.max(0, privateKey.length - 16)) + privateKey.substr(-8);
                ...
                statusEl.className = 'status-indicator connected';
                statusEl.style.backgroundColor = '#28a745'; // green
                log(`✅ Private key added successfully`);
                log(`📍 Address: ${result.address || 'N/A'}`);
                ...
                log(`❌ Failed to add private key: ${e.message}`, true);
            }
        }
        ...
        async function createRawTransaction() {
            try {
                ...
                if (!recipientEl || !amountEl || !coinEl || !statusEl || !rawTxEl || !endpointEl || !walletNameEl) {
                    log('❌ Transaction UI elements missing', true);
                    return;
                }
                ...
                if (!recipient || !amount || amount <= 0) {
                    log('❌ Invalid recipient address or amount', true);
                    return;
                }
                ...
                statusEl.className = 'status-indicator';
                statusEl.style.backgroundColor = '#f5a623';
                log(`📝 Creating raw transaction: ${amount} ${coin} to ${recipient}`);
                ...
                if (result.rawTransaction) {
                    ...
                    log(`🔗 Transaction ID: ${result.txId || 'N/A'}`);
                }
                ...
                log(`❌ Failed to create raw transaction: ${e.message}`, true);
            }
        }
        ...
        async function signTransaction() {
            try {
                ...
                if (!rawTransaction) {
                    log('❌ No raw transaction to sign', true);
                    return;
                }
                ...
                statusEl.className = 'status-indicator';
                statusEl.style.backgroundColor = '#f5a623';
                log(`✍️ Signing transaction with wallet '${walletName}'`);
                ...
                log(`❌ Failed to sign transaction: ${e.message}`, true);
            }
        }
        ...
        async function broadcastTransaction() {
            try {
                ...
                if (!signedTransaction) {
                    log('❌ No signed transaction to broadcast', true);
                    return;
                }
                ...
                log(`🔗 Transaction hash: ${result.txHash || 'N/A'}`);
                ...
                log(`❌ Failed to broadcast transaction: ${e.message}`, true);
            }
        }
        ...
        async function getRandomStunServers(maxCount = 1) {
            try {
                ...
                log(`🌐 Fetching public STUN list from ${url}`);
                ...
                log(`🔧 Using ${pick.length} STUN server(s)`);
                return [{ urls: pick }];
            } catch (e) {
                log(`⚠️ STUN list fetch failed: ${e.message}. Falling back to host-only ICE.`, true);
                return [];
            }
        }
        async function hostGame() {
            log('🎮 Starting multiplayer host...');
            isHost = true;
            ...
            if (!cfg.iceServers || cfg.iceServers.length === 0) {
                log('🛡️ Using host-only ICE (no STUN/TURN). Connectivity may be limited behind NAT.');
            } else {
                log(`🔧 Using custom ICE config with ${cfg.iceServers.length} server(s).`);
            }
            ...
            log('📨 Share this Game ID with your friend to join:');
            log(JSON.stringify(inviteObj));

            // Also log a truncated raw offer for debugging
            log('🌐 Host ready. Scan the QR or share the invite URL below.');
        ...
        async function joinGame() {
            ...
            if (!gameId) {
                log('❌ Please enter a game ID', true);
                return;
            }
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            log('🎮 Production Decentralized DOOM Loader Ready');
            log('📡 Click "Verify EmerDNS Domains" to begin');
            ...
            if (coinEl) {
                coinEl.addEventListener('change', () => {
                    ...
                    endpointEl.value = '';
                    log('ℹ️ RPC endpoint must be provided explicitly. No default applied.');
                });
            }
            ...
            if (walletInput) {
                walletInput.addEventListener('change', () => {
                    ...
                    reader.onload = () => {
                        ...
                        log(`🔑 Wallet loaded: ${f.name} (${walletBytes.byteLength} bytes)`);
                    };
                    reader.onerror = () => {
                        walletBytes = null;
                        if (walletStatus) walletStatus.style.backgroundColor = '#e74c3c';
                        log('❌ Failed to read wallet file', true);
                    };
